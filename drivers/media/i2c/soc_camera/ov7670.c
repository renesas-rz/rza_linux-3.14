...
static int ov7670_g_crop(struct v4l2_subdev *sd, struct v4l2_crop *a)
{
	a->c.left	= 0;
	a->c.top	= 0;
	a->c.width	= VGA_WIDTH;
			...
			struct v4l2_mbus_framefmt *mf)
{
	...
	// todo use curreent format for width and height
	mf->width	= VGA_WIDTH;
...
static int ov7670_s_stream(struct v4l2_subdev *sd, int enable)
{
	return 0;
}
			...
			const struct i2c_device_id *id)
{
	...
	if (!ssdd || !ssdd->drv_priv) {
		dev_err(&client->dev, "OV7670: missing platform data!\n");
		return -EINVAL;
	}

	config = ssdd->drv_priv;
	if (!config) {
		dev_err(&client->dev, "OV7670 driver needs config data\n");
		return -EINVAL;
	}
